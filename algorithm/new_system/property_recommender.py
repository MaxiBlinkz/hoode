...
class PropertyRecommender:
    ...
    def prepare_features(self, properties_df, weights):
        ...
        # Create feature list starting with scaled numerical features
        feature_components = [scaled_features * weights['price']]
        
        # Add property type features
        feature_components.append(property_type_dummies.values * weights['property_type_score'])
        
        # Add location score if available
        if 'location_score' in weights and 'location_score' in properties_df:
            feature_components.append(properties_df['location_score'].values.reshape(-1, 1) * weights['location_score'])
        
        # Combine all features
        weighted_features = np.column_stack(feature_components)
